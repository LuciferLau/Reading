...
# 第13章 网络通信：数据报文的发送
> 本章介绍了几个发送数据的系统调用(send/sendto/sendmsg)，其底层都是调用协议族的 sendmsg 接口
>
> 而TCP和UDP最终调用的IP层接口是不一样的，因为前者面对字节流，后者面向数据块

---

# 第14章 网络通信：数据报文的接收
> 本章介绍了几个发送数据的系统调用(recv/recvfrom/recvmsg)，其底层都是调用协议族的 recvmsg 接口
>
> 额外还提到了更为底层的网卡驱动相关的部分实现

# 第15章 编写安全无错代码
1、不要用 memcmp 对比结构体，可能因为内存对齐得出错误结果

2、有无符号数对位移的影响，有符号数使用算数左右移动 SAL/SAR 用符号位填充；而无符号数逻辑左右移动 SHL/SHR 用0填充

3、数组和指针大部分情况下可以互换，数组占用的空间是其中元素占用的空间之和，而指针只看所指对象的大小，注意区分是指针本身还是所指对象

4、不同类型的整数进行比较时，小心隐式类型转换

5、volatile 表示变量易变，编译器不要对其优化，一般用于①外设寄存器映射的内存②多线程/异步访问的全局变量③嵌入式编程

但需要注意，它不提供原子性的保证，在多线程操作变量时还是得使用原子操作/加锁

6、浮点数比较需要使用范围比较，精度问题决定了，只能用近似而非完全相等，并且存在合法的特殊值 inf & nan

7、Intel 32位机器上移位指令只有5位，执行 >5 的位移时间，实际上移动位数 = N & 0x1F
